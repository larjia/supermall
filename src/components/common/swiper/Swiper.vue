...
export default {
  ...
  methods: {
    ...
    handleDom: function () {
      ...
      const sliderEls = swiperEl.getElementsByClassName('slide')
      ...
      if (this.slideCount > 1) {
        const cloneFirst = sliderEls[0].cloneNode(true)
        const cloneLast = sliderEls[this.slideCount - 1].cloneNode(true)
    ...
    touchEnd: function (e) {
      ...
      if (this.distance === 0) {
        return
      } else if (this.distance > 0 && currentMove > this.totalWidth * this.moveRatio) { // 向右移动超过moveRatio
        this.currentIndex--
